...
from src.torch_utils import train_reconstructor, test_reconstructor, torch_bures_distance
...
def calculate_single_run_metrics(train_loader: DataLoader, test_loader: DataLoader, measurement_subset_len: int, dir_name: str):
    num_qubits = 2
    measurement_subset = random.sample(range(len(Kwiat.basis)**num_qubits), measurement_subset_len)
    model_params = {
        'input_dim': measurement_subset_len*(num_qubits*2*2*2 + 1),
        'num_qubits': num_qubits, # 'Kwiat' basis matrices
        'layers': 6,
        'hidden_size': 64,
    }
    model = DensityMatrixReconstructor(**model_params)
    ...
    for epoch in range(1, num_epochs + 1):
        train_metrics = train_reconstructor(model, device, train_loader, optimizer, epoch, criterion=criterion, log_interval=10, measurements_subset=measurement_subset)
        test_metrics = test_reconstructor(model, device, test_loader, criterions, measurements_subset=measurement_subset)
...
if __name__ == '__main__':
    ...
    log_path = f'./logs/density_matrix_reconstructor_measurements_subset.log'
    ...
    for num_measurements in range(1, max_num_measurements + 1):
        ...
        dir_name = f'density_matrix_reconstructor_m{num_measurements}'
        metrics = {
            'test_loss_avg': 0,
        ...
        for _ in range(num_repetitions):
            loss, bures_distance = calculate_single_run_metrics(train_loader, test_loader, num_measurements, dir_name)
        ...
        write_mode = 'w' if num_measurements == 1 else 'a'
        log_metrics_to_file(metrics, log_path, write_mode=write_mode, xaxis=num_measurements, xaxis_name='num_measurements')
    plot_metrics_from_file(log_path, title='Metrics', save_path=f'./plots/density_matrix_reconstructor_measurements_subset_metrics.png')