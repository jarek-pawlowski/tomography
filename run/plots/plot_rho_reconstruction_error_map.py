...
hlp_results_path = './logs/1qbit/density_matrix_reconstructor_from_hlp/complex_distance_reconstruction_from_m{}.log'
mean_results_path = './logs/1qbit/density_matrix_reconstructor_from_mean/complex_distance_reconstruction.log'
...
hlp_plot_path = './plots/final_results/1qbit_hlp_complex_distance.png'
mean_plot_path = './plots/final_results/1qbit_mean_complex_distance.png'
...
hlp_metrics_values = np.nan * np.ones(((num_measurements - 1) ** 2, len(metrics_names)))
mean_metrics_values = np.nan * np.ones(((num_measurements - 1) ** 2, len(metrics_names)))
...
for i in range(num_measurements):
    for j in range(i + 1, num_measurements):
        ...
        hlp_metrics = load_metrics_from_file(hlp_results_path.format(measurement_subset_str))
        hlp_metrics_values[i * (num_measurements - 1) + j - 1, :] = [hlp_metrics[metric_name][0] for metric_name in metrics_names]
