...
def calculate_single_run_metrics(train_loader: DataLoader, test_loader: DataLoader, measurement_subset_len: int, dir_name: str, model_input_info: str):
    ...
    for epoch in range(1, num_epochs + 1):
        train_metrics = train_tomography_corrections_predictor(model, device, train_loader, optimizer, epoch, criterion=criterion, log_interval=10, measurements_subset=measurement_subset, model_input_info=model_input_info)
        test_metrics = test_tomography_corrections_predictor(model, device, test_loader, criterions, measurements_subset=measurement_subset, model_input_info=model_input_info)
