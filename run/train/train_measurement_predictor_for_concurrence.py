...
from src.model import SequentialMeasurementPredictor, RecurrentMeasurementPredictor, LSTMMeasurementPredictorForConcurrence, SequentialMeasurementPredictorForConcurrence
...
def main():
    ...
    # create model
    model_name = 'smp_measure_basis_concurrence'
    ...
    model = SequentialMeasurementPredictorForConcurrence(**model_params)
    ...
    for epoch in range(1, num_epochs + 1):
        train_metrics = train_measurement_predictor(model, device, train_loader, optimizer, epoch, criterion=criterion, log_interval=10, mode='concurrence')
        test_metrics = test_measurement_predictor(model, device, test_loader, criterions, model_params['max_num_measurements'], mode='concurrence')
