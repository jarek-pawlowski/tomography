...
from src.test_heuristics import test_reconstruction_from_noisy_measurements
...
for i in range(0, 1):
    ...
    for j in tqdm(range(num_repetitions_i), desc='Averaging metrics...'):
        ...
        test_metrics = test_reconstruction_from_noisy_measurements(test_loader, criterions, varying_input_idx=measurement_ids, noise_variance=1., strategy=strategy, method=method, use_intensity=use_intensity)
        if i > 0:
