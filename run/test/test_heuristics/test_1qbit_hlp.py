...
def calculate_single_run_metrics(dir_name: str, test_loader: DataLoader, measurement_subset_len: int, num_qubits: int = 2, previously_used_measurements: list = []):
    ...
    test_metrics = test_1qbit_hlp(test_loader, criterions, measurements_subset=list(measurement_subset), device=device)
...
if __name__ == '__main__':
    ...
    num_qubits = 1
    dir_name = f'./logs/1qbit/density_matrix_reconstructor_from_hlp/'
    log_path = f'./logs/1qbit/density_matrix_reconstructor_from_hlp.log'
    ...
    for num_measurements in range(min_num_measurements, max_num_measurements + 1):
        ...
        for _ in range(num_repetitions):
            if len(used_measurements) == num_possible_measurements:
                break
            try:
                mse_loss, rmse_loss, bures_distance = calculate_single_run_metrics(dir_name, test_loader, num_measurements, num_qubits=num_qubits, previously_used_measurements=used_measurements)
    ...
    plot_metrics_from_file(log_path, title='Metrics', save_path=f'./plots/1qbit/density_matrix_reconstructor_from_hlp.png', xaxis='num_measurements')